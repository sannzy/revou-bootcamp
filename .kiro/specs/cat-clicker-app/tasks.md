...
- [ ] 2. Pastel gradient background and centered layout
  - [-] 2.1 Add full-viewport pastel gradient background and Flexbox centering
...
- [ ] 3. Cat inline SVG + button wrapper
  - [~] 3.1 Create the cat button and SVG illustration
...
- [ ] 4. CSS keyframes for animations
  - [~] 4.1 Write all animation keyframes in the `<style>` block
...
- [ ] 5. Speech bubble component markup
  - [~] 5.1 Add the speech bubble element to HTML and style it
...
- [ ] 6. Pat counter display
  - [~] 6.1 Add the Pat Counter HTML element with Glassmorphism styling
...
- [ ] 7. Milestone banner component markup
  - [~] 7.1 Add the Milestone Banner HTML element
...
- [ ] 8. JavaScript constants and state
  - [~] 8.1 Define `REACTIONS` array and `MILESTONES` set in the `<script>` block
...
- [ ] 9. `selectReaction()` function with no-repeat logic
  - [~] 9.1 Implement `selectReaction(pool, lastId)` pure function
...
- [ ] 10. `cancelActiveReaction()` and reaction application functions
  - [~] 10.1 Implement `cancelActiveReaction()`, `applyAnimationReaction(reaction)`, and `applySpeechReaction(reaction)`
...
- [ ] 11. `renderCounter()` and `incrementCounter()` functions
  - [~] 11.1 Implement counter rendering with pop animation
...
- [ ] 12. `getMilestoneBannerText()` and `showMilestoneBanner()` functions
  - [~] 12.1 Implement milestone text helper and banner display
...
- [ ] 13. `persistCount()` and `loadCount()` — localStorage persistence
  - [~] 13.1 Implement `loadCount()` and `persistCount(count)` with error handling
...
- [ ] 14. `handlePet()` orchestration function
  - [~] 14.1 Implement `handlePet()` as the single entry-point for all pet events
...
- [ ] 15. App initialisation and event wiring
  - [~] 15.1 Implement `initApp()` and attach all event listeners
...
- [ ] 16. Responsive sizing verification and viewport-safe speech bubble
  - [~] 16.1 Verify and fix responsive cat sizing and speech bubble overflow
